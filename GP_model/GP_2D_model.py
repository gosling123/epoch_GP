...
class GP_class:
    ...
    def __init__(self, X, y, y_var, kern=['RBF'], kern_ops=[],\
        ...
        if self.n_inputs != len(self.kern_var):
            sys.exit('(ERROR) : Please ensure number of kernels (kern_var) is equal in length to number of input parameters')
    ...
    def test_train_plots(self, model):
        ...
        plt.savefig(f'2D_{model}_test_train.png')
